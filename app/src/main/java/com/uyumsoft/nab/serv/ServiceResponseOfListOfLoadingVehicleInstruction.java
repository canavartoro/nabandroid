...
public class ServiceResponseOfListOfLoadingVehicleInstruction extends BaseObject {

     public ArrayOfString Errors = new ArrayOfString();  //array
     public ArrayOfLoadingVehicleInstruction Item = new ArrayOfLoadingVehicleInstruction();  //array
    ...
    public void getPropertyInfo(int index, Hashtable properties, PropertyInfo info) {
    	...
    	switch (index) {
           ...
           case 1: 
                info.name = "Item"; 
                info.type = new ArrayOfLoadingVehicleInstruction().getClass(); 
                             break; 
    ...
    public void setProperty(int index, Object value)
    {
    	switch (index)
    	{
           ...
           case 1: 
              if(value != null){ 
                  Item = new ArrayOfLoadingVehicleInstruction(); 
                  SoapObject prp = (SoapObject)value; 
                  int itemCount = prp.getPropertyCount(); 
                  for(int loop=0;loop<itemCount;loop++){ 
                  if(prp.getProperty(loop) instanceof SoapObject){ 
    ...
    {
        envelope.addMapping(NAMESPACE, "ServiceResponseOfListOfLoadingVehicleInstruction", this.getClass());
                   new ArrayOfString().register(envelope); 
           new ArrayOfLoadingVehicleInstruction().register(envelope); 
