...
public class ServiceResponseOfListOfShippingCode extends BaseObject {

     public ArrayOfString Errors = new ArrayOfString();  //array
     public ArrayOfShippingCode Item = new ArrayOfShippingCode();  //array
    ...
    public void getPropertyInfo(int index, Hashtable properties, PropertyInfo info) {
    	...
    	switch (index) {
           ...
           case 1: 
                info.name = "Item"; 
                info.type = new ArrayOfShippingCode().getClass(); 
                             break; 
    ...
    public void setProperty(int index, Object value)
    {
    	switch (index)
    	{
           ...
           case 1: 
              if(value != null){ 
                  Item = new ArrayOfShippingCode(); 
                  SoapObject prp = (SoapObject)value; 
                  int itemCount = prp.getPropertyCount(); 
                  for(int loop=0;loop<itemCount;loop++){ 
                  if(prp.getProperty(loop) instanceof SoapObject){ 
                  SoapObject pi = (SoapObject)prp.getProperty(loop); 
                      ShippingCode item = new ShippingCode(); 
    ...
    {
        envelope.addMapping(NAMESPACE, "ServiceResponseOfListOfShippingCode", this.getClass());
                   new ArrayOfString().register(envelope); 
           new ArrayOfShippingCode().register(envelope); 
